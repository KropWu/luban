<template>
    <div style="margin-bottom: 16px">
        <a-space>
            <a-button type="primary" @click="addK8SCluster">新增用户</a-button>
            <a-button type="primary" danger :disabled="state.selectedRows.length <= 0"
                @click="removeCluster()">批量删除</a-button>
        </a-space>
    </div>
    <div>
    ...
    <a-spin :spinning="state.loading" size="large">
        <a-table :row-selection="rowSelection" :columns="columns" :data-source="state.data" :pagination="false" rowKey="id"
            ...
            <template #kubeConfig="{ text, id }">
                <a-tooltip placement="topLeft" title="查看凭证">
                    <a @click="ViewClusterConfig(id, text.id)">
                        <IconFont type="pigs-icon-pingzheng" />
                    </a>
                </a-tooltip>
            </template>
        ...
        <a-modal v-model:visible="state.ClusterConfigVisible" title="查看集群凭证" :footer="null" :keyboard="false"
            :maskClosable="false">

            <a-textarea v-model:value="state.ClusterConfig" placeholder="请粘贴KubeConfig内容"
        ...
        <div class="float-right" style="padding: 10px 0;">
            ...
            <a-pagination size="md" :show-total="total => `共 ${state.total} 条数据`" :v-model="state.total"
                :page-size-options="state.pageSizeOptions" :total="state.total" show-size-changer :pageSize="state.pageSize"
                show-less-items align="right" @showSizeChange="onShowSizeChange" @change="onChange">
